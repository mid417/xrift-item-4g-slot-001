import { getActivePaylines, readPayline } from './paylines.js';
...
function symbolAt(strip, centerIndex, rowOffset) {
    return strip[wrapIndex(centerIndex - rowOffset, strip.length)];
}
export function visibleBoardFromCenterIndices(centerIndices) {
    return [
        [
...
function createReplayOutcome(bet) {
    return {
        kind: 'REPLAY',
        payout: bet,
        title: 'REPLAY',
...
export function resolveSpinBet(bet, lastOutcome) {
    if (bet > 0) {
        return bet;
    }
    return lastOutcome.kind === 'REPLAY' ? lastOutcome.payout : 0;
}
export function evaluateBoard(centerIndices, bet) {
...
export function getReachableStopIndices(reelIndex, currentPosition) {
...
export function resolveStopIndex({ reelIndex, currentPosition, lockedCenterIndices, bet, pendingKind, bonusFlag, }) {
...
export function resolveBonusFlag(currentBonusFlag, outcomeKind) {
    if (outcomeKind === 'BIG' || outcomeKind === 'REG') {
        return null;
    }
    return currentBonusFlag;
}
function buildDesiredKinds(pendingKind, bonusFlag) {
    const desiredKinds = [];
    if (pendingKind !== 'MISS') {
...
function replaceCenterIndex(centerIndices, reelIndex, centerIndex) {
    const nextCenterIndices = [...centerIndices];
    nextCenterIndices[reelIndex] = centerIndex;
    return nextCenterIndices;
}
function canResolveOutcome(centerIndices, desiredKind, bet) {
